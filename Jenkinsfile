		...
		stage('Static Code Analysis') {      
			steps{        
				...
				withSonarQubeEnv('Sonar') {
					sh "${tool name: 'SonarScanner', type: 'hudson.plugins.sonar.SonarRunnerInstallation'}/bin/sonar-scanner -Dsonar.projectKey=parking.${BRANCH_NAME} -Dsonar.projectName=parking.${BRANCH_NAME} -Dproject.settings=./sonar-project.properties"
				}
			}
		}
	}
